...
Core Features
Prompt and Command Execution: Basic command-line interface with prompt and command execution.
Argument Parsing: Commands with multiple arguments are parsed and executed correctly.
Background Process Management: Commands followed by & run as background processes.
Command History: Stores up to 10 previous commands, accessible with !number.
Built-In Commands:
...
User-Defined Variables: Support for setting, retrieving, and listing local variables using name=value syntax.
...
Variable Deletion: Currently, there is no mechanism to unset or delete user-defined variables.
Error Handling: Error messages may not always be specific, especially for syntax errors in complex commands.
Multiple Redirections: The shell does not support multiple input or output redirections in a single command (e.g., command < input > output).
Multiple Piping: Limited support for multiple pipes in a single command (e.g., command1 | command2 | command3 may not work as expected).
Acknowledgments
...
GeeksforGeeks: Helped with basic shell implementation techniques and understanding of processes.
GNU C Library Documentation: Provided in-depth details on UNIX system calls.
Stack Overflow: Assisted with troubleshooting specific errors related to process management and variable handling.
